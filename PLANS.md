...
Residual risks: remote GitHub Actions execution has not run because no commit or push was authorized. The repository now has `HEAD` at the bootstrap commit, while Milestone 1 remains an intentionally uncommitted working tree; local build evidence therefore uses the truthful `uncommitted` sentinel and security scans the reachable bootstrap history plus current changes. Garage exposes no analyzable package inventory and remains local-only; release SBOM/signature/provenance evidence is absent; the waiver-free production scan exposes `CVE-2026-14456` in PostgreSQL and Redis, while their exact local-only waivers must be removed by 2026-09-30. Live identity verification, production infrastructure, native releases, and production security evidence remain future work.
...
Targeted Batch 1 evidence was refreshed on 2026-08-29 after Docker recovery. `corepack pnpm run secrets` exited 0 from the exact trusted project path after verifying the pinned Gitleaks manifest, generated-token canary, unrelated same-file credential, nested `.env.local` credential, disposable prior-history credential, four exact working-tree fixtures, and the then-unborn repository. The exact repository path, not a wildcard, is present in Git `safe.directory`.
...
Final Batch 3 verification on 2026-08-29: `corepack pnpm run verify`, `security`, and `smoke` all exited 0. `verify` included generation/format freshness, lint/typechecks, Go and JavaScript tests, 23/23 mobile tests, executable contracts, the complete real-container integration suite, command builds, the Next.js production build, and Expo Android export. `security` again passed every pinned secret canary/history/working-tree check, reported no `govulncheck`, `gosec`, or complete-graph pnpm audit findings, inventoried 53 PostgreSQL and 22 Redis packages under their exact local waivers, and continued to report Garage as missing coverage/local-only. Smoke passed health/readiness/version, authenticated GET/PATCH, unauthenticated 401, route logging, and build metadata with `version=0.1.0-bootstrap`, `commit=uncommitted`, and `builtAt=2026-08-29T14:38:05.370Z`. The immutable `golang:1.26.7-bookworm@sha256:e8c859f5632dcfde7b32d2012b4351728f6437930887c2f6a91ea242459e5514` Linux `go test -race ./...` gate exited 0 with the repository mounted read-only. The first sandboxed `verify` attempt could not spawn Docker; its approved rerun is the recorded passing gate. This evidence predates Milestone 1.

## Milestone 1 — authentication and resumable onboarding

Status: **complete locally — manual Auth0, browser, and native-device evidence pending**.

Implemented scope:

- Auth0 native Authorization Code + PKCE integration, protected refresh credentials, process-death restoration, recoverable offline/expiry/logout states, subject-isolated query caches, and no-sensitive-value analytics events.
- Auth0 Next.js v4 encrypted HttpOnly server session, fixed callback routing, server-only access-token refresh, and a same-origin BFF with exact-origin/Fetch Metadata, strict media/idempotency, bounded bodies, fixed upstream, and safe cookie/error behavior.
- Fail-closed Go OIDC/JWKS verifier for issuer/audience/client/signature/RS256 key metadata and rotation/time/subject/email/scope claims; deterministic auth remains local/test only.
- Scope-protected, owner-resolved onboarding persistence with serialized first login, strict validation, nullable optional diet clearing, authoritative owner-scoped idempotency replay/conflict, preserved adult/safety evidence, and consent-version re-gating.
- Mobile and web restoration/edit/offline/error/retry/completion states; OpenAPI 3.1 and generated client updates; migration/sqlc output; unit/component/contract/concurrency/PostgreSQL coverage; runbook, ADR, threat/data/evidence/readiness updates.

Scope deliberately excluded: workout catalog, workout generation/logging, progression, nutrition calculations, billing, admin tools, deployment, and production activation.

Manual blockers before this milestone can be considered operationally verified: Auth0 tenant callback/logout/API permission and verified-email settings; native Android/iOS development-client login, protected storage, refresh, logout, offline, and process-death cases; web browser callback/session/cookie behavior; second-user cache isolation on device; and legal review of Terms, Privacy, adult/safety language, retention, and consent evidence. No Auth0 dashboard was accessed or changed during this work.

Final local evidence on 2026-08-30: frozen installation, formatting and generated-code freshness, lint, typecheck, unit/component/contract tests, PostgreSQL/Redis/object-store integration, builds, security, migration, smoke, and the final `corepack pnpm verify` gate all exited 0. The final gate passed doctor/config/Compose validation, OpenAPI generation drift, Go vet, all Go packages, 12/12 tooling tests, 3/3 generated-client tests, 21/21 web tests, 53/53 mobile tests, the complete real-container integration suite, all Go command builds, the Next.js production build, and Expo Android export. Security found no unallowlisted secret, reachable-history secret, Go vulnerability, gosec finding, or pnpm advisory; the local image gate retained only the exact expiring PostgreSQL/Redis waivers and the explicit Garage missing-inventory/local-only block. Smoke passed profile and onboarding authentication, persistence, authoritative identical replay, mismatched-key conflict, exact CORS allow/deny, and `version=0.2.0-milestone1` with the truthful `commit=uncommitted` dirty-tree marker. No Auth0 dashboard, commit, push, production data, or workout functionality was used.

## Future milestones

1. `complete locally; manual Auth0/device evidence pending` — OIDC authentication and resumable adult onboarding.
...
Bootstrap and local Milestone 1 completion are not production readiness. Live OIDC/device verification, broader authorization, offline workout behavior, production infrastructure, release builds, DAST, independent testing, legal review, and operational drills remain explicitly incomplete.
